...
AddEventHandler(
  ...
  function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent("ddx_trunk_inventory:getOwnedVehicule")
...
RegisterNetEvent("ddx_trunk_inventory:setOwnedVehicule")
AddEventHandler(
  "ddx_trunk_inventory:setOwnedVehicule",
  function(vehicle)
...
function openmenuvehicle()
  ...
  if not IsPedInAnyVehicle(playerPed) then
    ...
    if not Config.CheckOwnership or (Config.AllowPolice and PlayerData.job.name == "police") or (Config.CheckOwnership and myVeh) then
      if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
        ...
        if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            ESX.ShowNotification('Le coffre du véhicule est ~r~fermé')
            end
          end
        else
          ESX.ShowNotification('Aucun véhicule a ~r~proximité')
        end
        lastOpen = true
      ...
      -- Not their vehicle
      ESX.ShowNotification('Ce véhicule ne vous appartient ~r~pas')
    end
  end
end
local count = 0
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    "ddx_trunk:getInventoryV",
    function(inventory)
      text = _U("trunk_info", plate, (inventory.weight / 1000), (max / 1000))
      data = {plate = plate, max = max, myVeh = myVeh, text = text}
      TriggerEvent("ddx_inventoryhud:openTrunkInventory", data, inventory.blackMoney, inventory.items, inventory.weapons)
    end,
    plate
  )
end
