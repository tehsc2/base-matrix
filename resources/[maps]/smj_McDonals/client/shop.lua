    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
	pNotify("Pulsa [L] para mirar tu pedido.", "error", 2500)
                    if dist >= 12.0 then
                        pNotify("Eliminaste tu pedido, tu pedido está vacío!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("No tienes nada en tu pedido!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            ...
            {label = 'Importe a pagar: <span style="color:green">$' .. payAmount ..'</span>'},
        }
                    ...
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'penis2',
                        {
                            ...
                            elements = {
                                ...
                                {label = "Pagar con VISA", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('McDonals:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    TriggerServerEvent('McDonals:Cashier', payAmount, Basket, data2.current["value"])
                                    payAmount = 0
                                    Basket = {}
                                    menu2.close()
                                else
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Poniendo " .. data.current.value .. " unidades de " .. data.current["realLabel"] .. " en la cesta", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'Importe a pagar: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Eliminar todo de tu cesto", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Carrito de la Compra - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. "unidades",
                            align    = 'center',
                            elements = {
                                {label = shopItem["label"] .. " - $" .. shopItem["price"] * shopItem["amount"]},
                                {label = '<span style="color:red">Eliminar objeto</span>', value = "deleteItem"},
                            },
                        },
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Removido " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " de la cesta", "alert", 2500)
