...
DrawText3D = function(x, y, z, text)
    ...
    if onScreen then
        ...
        DrawText(x,y)
        DrawRect(x,y + 0.0125, 0.025 + factor, 0.03, 41, 11, 41, 0)
    end
end
...
--                SetBlipSprite(blip["21"], 446)
--                SetBlipDisplay(blip["id"], 4)
--                SetBlipScale(blip["id"], 0.6)
--                SetBlipColour(blip["id"], 17)
...
AddEventHandler('onResourceStop', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        TriggerServerEvent('esx:clientLog', "[Tiendas]: Eliminando npcs...")
        DeleteCashier()
    end
end)
