...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('McAuto:requestDBItems', function(ShopItems)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				TriggerServerEvent('McAuto:buyItem', data.current.item, data.current.value, zone)
			end
...
AddEventHandler('McAuto:hasEnteredMarker', function(zone)
	CurrentAction     = 'shop_menu'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('McAuto:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Zones) do
		for i = 1, #v.Pos, 1 do
			...
			AddTextComponentString('~y~Mc~r~Auto')
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('McAuto:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('McAuto:hasExitedMarker', LastZone)
		end
	end
end)
			...
			if IsControlJustReleased(0, 38) then
					...
					if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then 
						...
						CurrentAction = nil
                    					...
                    					ESX.ShowNotification("~o~Tienes que estar en un vehículo para pedir en el McAuto.")
                				end
...
RegisterNetEvent('McAuto:clipcli')
AddEventHandler('McAuto:clipcli', function()
  ped = GetPlayerPed(-1)
  if IsPedArmed(ped, 4) then
    hash=GetSelectedPedWeapon(ped)
    if hash~=nil then
      TriggerServerEvent('McAuto:remove')
      AddAmmoToPed(GetPlayerPed(-1), hash,25)
      ESX.ShowNotification("Has utilizado un cargador.")
    else
      ESX.ShowNotification("No tienes ningún arma en la mano.")
    end
  else
    ESX.ShowNotification("Esa munición no vale para ese arma.")
  end
end)
...
RegisterNetEvent('McAuto:silencieux')
AddEventHandler('McAuto:silencieux', function(duration)
...
RegisterNetEvent('McAuto:flashlight')
AddEventHandler('McAuto:flashlight', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local flashlight = 0
					for i=1, #inventory, 1 do
...
RegisterNetEvent('McAuto:grip')
AddEventHandler('McAuto:grip', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local grip = 0
					for i=1, #inventory, 1 do
...
RegisterNetEvent('McAuto:yusuf')
AddEventHandler('McAuto:yusuf', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local yusuf = 0
					for i=1, #inventory, 1 do
